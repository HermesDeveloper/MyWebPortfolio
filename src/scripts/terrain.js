const THREE_CDN = "https://cdn.jsdelivr.net/npm/three@0.186.1/build/three.module.js";
...
function createTerrainGeometry(THREE) {
...
async function initializeTerrainHost(host) {
  const canvas = host.querySelector(".terrain-canvas");
  if (!canvas || host.dataset.terrainInitialized === "true") return;
  ...
  try {
    ...
    const geometry = createTerrainGeometry(THREE);
    ...
    const applyZoom = (scale) => {
      camera.position.setLength(THREE.MathUtils.clamp(camera.position.length() * scale, 5.5, 18));
      camera.lookAt(0, 0, 0);
    };
    ...
    const onPointerMove = (event) => {
      ...
      if (activePointers.size > 1) {
        ...
        if (pinchDistance > 0 && nextDistance > 0) applyZoom(pinchDistance / nextDistance);
        pinchDistance = nextDistance;
      } else {
        terrain.rotation.y += (event.clientX - previous.x) * 0.006;
        terrain.rotation.x = THREE.MathUtils.clamp(
          terrain.rotation.x + (event.clientY - previous.y) * 0.004,
          -0.48,
          0.28,
        );
      }
    ...
    console.error("Unable to initialize the terrain visualization.", error);
...
export function initializeTerrain() {
  document.querySelectorAll("[data-terrain]").forEach((host) => {
    void initializeTerrainHost(host);
  });
}
